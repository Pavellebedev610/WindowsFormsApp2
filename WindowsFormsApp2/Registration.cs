...
namespace WindowsFormsApp2
{
    public partial class Registration : Form
    {
        ...
        {
            cn = new SqlConnection(@" Data Source = (LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Павел\Desktop\WindowsFormsApp2\WindowsFormsApp2\Database1.mdf;Integrated Security = True");
            cn.Open();
        }
        ...
        private void BtnRegister_Click(object sender, EventArgs e)
        {
            if (txtconfirmpassword.Text != string.Empty || txtpassword.Text != string.Empty || txtusername.Text != string.Empty || textmail.Text != string.Empty)
            {
                ...
                {
                    command = new SqlCommand("select * from Table_1 where Username='" + txtusername.Text + textmail.Text + "'", cn);
                    ...
                    {
                        dr.Close();
                        command = new SqlCommand("insert into Table (Username, Password, Mail) values(@Username,@Password,@Mail)", cn);
                        command.Parameters.AddWithValue("Username", txtusername.Text);
                        command.Parameters.AddWithValue("Password", txtpassword.Text);
                        command.Parameters.AddWithValue("Mail", textmail.Text);
                        command.ExecuteNonQuery();
                        MessageBox.Show("Поздравляю, вы успешно зарегестрировались'.", "Done", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
